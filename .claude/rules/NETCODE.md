...
| Поиск сессий в локальной сети | `Code/App/LanDiscovery.cs` (на `Session.prefab`); UI — шаг `Search` в `Code/UI/MenuView.cs` | **Свой UDP-бикон мимо FishNet:** у Tugboat дискавери нет. Хост, пока поднят сервер, шлёт `RNDTA1:<порт>` на широковещательный адрес **каждого** интерфейса — лимитированный `255.255.255.255` уходит только в маршрут по умолчанию, а с поднятым VPN это не тот LAN. Адрес берётся из отправителя пакета, а не из полезной нагрузки. Приём — `BeginReceive` на пуле потоков, поэтому пакеты уезжают в очередь под `lock` и разбираются в `Update`; записи протухают по `entryTimeout`, сокеты закрываются в `OnDisable`/`OnApplicationQuit` — иначе следующий Host падает на занятом порту |
...
| Выбор локации и вылет | `Code/Hub/HubMenuUI.cs` → `RaidSession.StartRaid(sceneName)`; нажатие — `Code/Player/PlayerInteractor.cs` (`InteractionKind.Map`) | **Канала нет:** выбор — решение хоста, а клиенту приезжает уже загрузка сцены. Список локаций у клиента не строится. Карта — мировой канвас, и жмёт её тот же интерактор, что двери и животных: `HubBootstrapper` отдаёт локальному аватару `GraphicRaycaster` карты (`BindMap`), подсказка идёт в HUD аватара, а не на стену |
